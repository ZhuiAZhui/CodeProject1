...
const enum
{
	...
	MT_BEGIN_UPDATE_MESG,		// 服务端发送 开始升级 消息
	...
	MT_ENDPRORES_MESG,		// 客户端发送 结束进程 结果消息，由server接收：0--结束exe失败； 1--结束进程exe成功

	MT_UPDATE_MESG,			// 服务端发送 开始更新文件 消息，并发送文件等待客户端接收
	MT_UPDATERES_MESG,		// 客户端发送 接收结果消息，由server接收：0--接收文件失败； 1--接收文件成功

	MT_BEGINPRO_MESG,		// 服务端发送 启动进程exe 消息
	MT_BEGINPRORES_MESG,	// 客户端发送 启动进程exe结果 消息，由server接收：0--启动进程exe失败； 1--启动进程exe成功
};
