...
BOOL CGroupTalk::JoinGroup()
{
	...
	if(nRet != SOCKET_ERROR)
	{
		...
		pHeader->gt_fromtype = m_usMsgFrom;
...
int CGroupTalk::Send(char *szText, int nLen, DWORD dwRemoteAddr)
{
	...
	dest.sin_port = ::ntohs(m_usMultiPort);
	return ::sendto(m_sSend, szText, nLen, 0, (sockaddr*)&dest, sizeof(dest));
}


CGroupTalk::CGroupTalk(HWND hNotifyWnd,  DWORD dwMultiAddr, u_short usMultiPort,
	u_char type, DWORD dwLocalAddr, int nTTL)
{
	m_hNotifyWnd = hNotifyWnd;
	m_usMsgFrom = type;
	m_dwMultiAddr = dwMultiAddr;
	m_usMultiPort = usMultiPort;
...
DWORD WINAPI _GroupTalkEntry(LPVOID lpParam)
{
	...
	si.sin_port = ::ntohs(pTalk->m_usMultiPort);
...
void CGroupTalk::DispatchMsg(GT_HDR *pHeader, int nLen)
{
	...
	{	
		// 是否来自自己，如果是，则不处理
		if (strcmp(pHeader->szUser, m_szUser) == 0 && pHeader->gt_fromtype == this->m_usMsgFrom)
		{
			return;
		}
	...
	// 通知主窗口处理
	::SendMessageW(m_hNotifyWnd, WM_GROUPTALK, 0, (LPARAM)pHeader);
}



int CGroupTalk::SendText(char *szText, int nLen, u_char type, DWORD dwRemoteAddr)
{
	// 构建消息封包
	char buf[sizeof(GT_HDR) + 1024] = { 0 };
	GT_HDR *pHeader = (GT_HDR *)buf;
	pHeader->gt_type = type;
	pHeader->gt_fromtype = m_usMsgFrom;
